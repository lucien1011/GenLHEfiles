...
for MPROD in {1000,}; do
    if [ ! -d "${baseOutDir}${MODEL}${MPROD}" ]; then
        mkdir ${baseOutDir}${MODEL}${MPROD}
    fi
    python ${SCRIPT} ${MODEL}${MPROD} --in-dir /eos/cms/store/user/klo/SUSY/LHE/SMS-StopStop-bl/${MODEL}${MPROD}/ --slha ${JOBS}/${MODEL}${MPROD}/${MODEL}${MPROD}.slha --qcut-range 60 62 --outDir ${baseOutDir}${MODEL}${MPROD} --time 36000
done
